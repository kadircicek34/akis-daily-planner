...
try {
  ...
  await desktop.getByRole('button', { name: 'Hafta', exact: true }).click()
  assert(await desktop.locator('.weekly-grid > section').count() === 7, 'haftalık plan görünümü çalışıyor')
  await desktop.getByRole('button', { name: 'Ay', exact: true }).click()
  assert(await desktop.locator('.month-grid > button').count() === 42, 'aylık takvim görünümü çalışıyor')
  await desktop.getByRole('button', { name: 'Gün', exact: true }).click()
  ...
  await mobile.getByLabel('Tekrar').selectOption('weekly')
  ...
  assert(await mobile.evaluate(() => JSON.parse(localStorage.getItem('akis-tasks')).filter(task => task.title === 'Smoke test görevi').length === 53), 'tekrarlanan görev serisi arayüzden oluşturuluyor')
  await mobile.getByText('Ayarlar').last().click()
  assert(await mobile.getByText('Varsayılan ses').count() === 1, 'bildirim sesi ayarı kullanılabilir')
  await mobile.locator('.setting-row').filter({ hasText: 'İsteğe bağlı AI' }).getByRole('button').click()
  assert(await mobile.evaluate(() => localStorage.getItem('akis-ai')) === 'on', 'AI özelliği kullanıcı tarafından açılıp kapatılabiliyor')
