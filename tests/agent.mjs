...
try {
  ...
  const created = await client.callTool({ name: 'create_task', arguments: { title: 'Agent görevi', date: '2026-09-22', start: '10:00', duration: 45, priority: 'high', reminderMinutes: 15, notificationSound: 'bell', energy: 2, subtasks: [{ title: 'Taslak çıkar' }] } })
  ...
  assert(payload.tasks[0].reminderMinutes === 15 && payload.tasks[0].subtasks.length === 1, 'MCP ve REST yeni görev alanlarını kayıpsız paylaşıyor')
  ...
  const noAi = await fetch('http://127.0.0.1:4318/v1/ai/subtasks', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title: 'Plan yap' }) })
  assert(noAi.status === 400 && (await noAi.json()).error.includes('yapılandırılmadı'), 'AI endpointi yapılandırma yokken kapalı kalıyor')
} finally {
